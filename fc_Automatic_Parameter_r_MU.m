function [Parameter] = fc_Automatic_Parameter_r_MU(Xinput, Parameter, MU, idx)
...
%
% The update formula:
% r_new = (1 / (MU + 1)) * r_old + (MU / (MU + 1)) * X_new
...
idxx = size(vParameter_r_Unsupervise(idx).MU, 1);
% Store the updated 'r' values in the tracking variable
vParameter_r_Unsupervise(idx).MU(idxx + 1, :) = change;
