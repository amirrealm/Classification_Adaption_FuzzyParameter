function [Parameter, NK_One] = fc_Automatic_Parameter_r_Nk_One(Xinput, Parameter, NK_One, idx)
% This function updates the 'r' parameter using an adaptive method based on NK_One.
% The update formula dynamically adjusts the weight factors based on NK_One(idx,1).
%
% Update formula:
% r_new = (NK_One / (NK_One + 1)) * r_old + (1 / (NK_One + 1)) * X_new
...
idxx = size(vParameter_r_Unsupervise(idx).NK_One, 1);
% Store the updated 'r' values in the tracking variable
vParameter_r_Unsupervise(idx).NK_One(idxx + 1, :) = change;
